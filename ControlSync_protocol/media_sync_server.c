...
* 1) start the synchronization control process it could evolve  the
	 PTP synchronization API or report to the slaves the static synchronization parameters .
...
void Ctrl_Sync_cinterval_IRQ_handler(){
  //uint8_t i;
	if(ctrl_peding_packet){
		CTRL_status = SYNCRONIZING;
  		switch(ctrl_sync_id)
